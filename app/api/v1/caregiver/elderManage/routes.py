...
router=APIRouter(prefix="/elders", tags=["Elder Management"])

    
@router.get("/{elder_id}")
def get_elder_details(elder_id: int,db: Session= Depends(get_db) #,current_user = Depends(verify_access_to_elder)
):
